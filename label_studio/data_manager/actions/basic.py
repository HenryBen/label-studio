...
def retrieve_tasks_predictions(project, queryset, **kwargs):
    ...
    return {'processed_items': queryset.count(), 'detail': _('Retrieved %(count)s predictions') % {'count': queryset.count()}}
...
def delete_tasks(project, queryset, **kwargs):
    ...
    return {'processed_items': count, 'reload': reload, 'detail': _('Deleted %(count)s tasks') % {'count': count}}
...
def delete_tasks_annotations(project, queryset, **kwargs):
    ...
    return {'processed_items': count, 'detail': _('Deleted %(count)s annotations') % {'count': count}}
...
def delete_tasks_predictions(project, queryset, **kwargs):
    ...
    return {'processed_items': count, 'detail': _('Deleted %(count)s predictions') % {'count': count}}
...
actions = [
    {
        ...
        'title': _('Retrieve Predictions'),
        'order': 90,
        'dialog': {
            'title': _('Retrieve Predictions'),
            'text': _('Send the selected tasks to all ML backends connected to the project.')
            + ' ' + _('This operation might be abruptly interrupted due to a timeout.')
            + ' ' + _('The recommended way to get predictions is to update tasks using the Label Studio API.')
            + ' ' + _('Please confirm your action.'),
            'type': 'confirm',
        },
    },
    {
        ...
        'title': _('Delete Tasks'),
        'order': 100,
        'reload': True,
        'dialog': {
            'text': _('You are going to delete the selected tasks. Please confirm your action.'),
            'type': 'confirm',
        },
    },
    {
        ...
        'order': 101,
        'dialog': {
            'text': _('You are going to delete annotations from the selected tasks.')
            + '\n' + _('You can select specific annotators to delete annotations for.')
            + '\n' + _('Please confirm your action.'),
            'type': 'confirm',
    ...
    },
    {
        ...
        'permission': all_permissions.predictions_any,
        'title': _('Delete Predictions'),
        'order': 102,
        'dialog': {
            'text': _('You are going to delete all predictions from the selected tasks. Please confirm your action.'),
            'type': 'confirm',
        },
    },
]
