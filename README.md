...
For frontend development and building, use the following tested configuration:

#### Environment Requirements
- **Node.js Version**: v18.20.8 (use `nvm use 18` to switch)
- **Package Manager**: npm (recommended over yarn for compatibility)
- **Working Directory**: `web/` subdirectory
...
```bash
# Serve the built application on port 3001
npx serve dist/apps/labelstudio -p 3001
```

#### Build Results
- **Build Time**: ~35 seconds
- **Output Directory**: `web/dist/apps/labelstudio`
- **Service URL**: `http://localhost:3001`
- **Status**: ✅ Builds successfully with 49 Sass warnings (non-critical)

#### Key Fixes Applied
1. **Translation Files**: Ensured consistency between `en.json` and `zh.json`
2. **Missing Dependencies**: Created `web/libs/datamanager/src/utils/i18n.js` for build compatibility
3. **Node.js Compatibility**: Using Node.js 18 to avoid syntax compatibility issues

> **Note**: Always use this exact configuration for consistent builds. Do not modify Node.js version or package manager.
