    ...
    <div className={importClass}>
      ...
      <header className="flex gap-4">
        <form
          ...
          onSubmit={onLoadURL}
        >
          <Input placeholder={t('importData.datasetUrl')} name="url" ref={urlRef} rawClassName="h-[40px]" />
          <Button variant="primary" look="outlined" type="submit" aria-label={t('importData.addUrl')}>
            {t('importData.addUrl')}
          </Button>
        </form>
        <span>or</span>
        <Button
          ...
          leading={<IconUpload />}
          aria-label={t('importData.aria.uploadFile')}
        >
          {files.uploaded.length ? t('importData.uploadMoreFiles') : t('importData.uploadFiles')}
        </Button>
        ...
        >
          <span>{t('importData.treatCsvAs')}</span>
          <label>
            <input {...csvProps} value="tasks" checked={csvHandling === "tasks"} /> {t('importData.listOfTasks')}
          </label>
          <label>
            <input {...csvProps} value="ts" checked={csvHandling === "ts"} /> {t('importData.timeSeriesOrWholeText')}
          </label>
        </div>
        <div className={importClass.elem("status")}>
          {files.uploaded.length ? t('importData.filesUploaded', { count: files.uploaded.length }) : ""}
        </div>
      </header>
          ...
          >
              ...
              <div className="flex gap-4 justify-center items-start w-full h-full">
                <label htmlFor="file-input" className="w-full h-full">
                  <div className={`${dropzoneClass.elem("content")} w-full`}>
                    ...
                    <header>
                      {t('importData.dragDropFiles')}
                      <br />
                      {t('importData.orClickToBrowse')}
                    </header>

                    <dl>
                      <dt>{t('importData.supportedFormats.images')}</dt>
                      <dd>{supportedExtensions.image.join(", ")}</dd>
                      <dt>{t('importData.supportedFormats.audio')}</dt>
                      <dd>{supportedExtensions.audio.join(", ")}</dd>
                      <dt>
                        <div className="flex items-center gap-1">
                          {t('importData.supportedFormats.video')}
                          <Tooltip title={t('importData.videoFormatSupportTooltip')}>
                            <a
                              href="https://labelstud.io/tags/video#Video-format"
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center"
                              aria-label={t('importData.aria.videoFormatSupport')}
                            >
                      ...
                      <dt>{t('importData.supportedFormats.htmlHypertext')}</dt>
                      <dd>{supportedExtensions.html.join(", ")}</dd>
                      <dt>{t('importData.supportedFormats.text')}</dt>
                      <dd>{supportedExtensions.text.join(", ")}</dd>
                      <dt>{t('importData.supportedFormats.structuredData')}</dt>
                      <dd>{supportedExtensions.structuredData.join(", ")}</dd>
                      <dt>{t('importData.supportedFormats.pdf')}</dt>
                      <dd>{supportedExtensions.pdf.join(", ")}</dd>
                    </dl>
                    <div className="tips">
                      <b>{t('importData.tips.important')}</b>
                      <ul className="mt-2 ml-4 list-disc font-normal">
                        <li>
                          {t('importData.tips.cloudStorageRecommended')}
                        </li>
                        <li>
                          {t('importData.tips.pdfMultiImage')}
                        </li>
                        <li>
                          {t('importData.tips.importPreannotated')}
                        </li>
                      </ul>
                    </div>
                  </div>
              ...
              <div className="w-full">
                ...
                >
                  <table className="w-full">
                    <thead>
                      <tr>
                        <th>{t('importData.filename')}</th>
                        <th>{t('importData.status')}</th>
                        <th>{t('importData.size')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {sample && (
            ...
            {ff.isFF(ff.FF_JSON_PREVIEW) && (
              <div className="w-full h-full flex flex-col min-h-[400px]">
                {projectConfigured ? (
                  <SimpleCard
                    title={t('importData.expectedInputPreview')}
                  ...
                  <SimpleCard className="w-full h-full flex flex-col items-center justify-center text-center p-wide">
                    <div className="flex flex-col items-center gap-tight">
                      ...
                      <div className="flex flex-col items-center gap-tighter">
                        <div className="text-label-small text-neutral-content font-medium">{t('importData.viewJsonFormat')}</div>
                        <div className="text-body-small text-neutral-content-subtler text-center">
                          {t('importData.setupLabelingConfigFirst')}{" "}
                          <Button
                            type="button"
                            look="string"
                            onClick={openConfig}
                          ...
                          >
                            {t('importData.labelingConfiguration')}
                          </Button>{" "}
                          {t('importData.toPreviewJsonFormat')}
                        </div>
                      </div>
                    </div>
                  </SimpleCard>
