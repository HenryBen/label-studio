...
export const StorageProviderForm = forwardRef<unknown, StorageProviderFormProps>(
  (
    ...
    const steps = isEditMode
      ? [
          { title: t('storage.configureConnection'), schema: getProviderSchema(type || "s3", isEditMode, effectiveTarget) },
          // Only include preview and review steps for import storages
          ...(effectiveTarget === "import"
            ? [{ title: t('storage.importSettingsPreview') }, { title: t('storage.reviewConfirm') }]
            : []),
        ]
      : [
          { title: t('storage.selectProvider'), schema: step1Schema },
          { title: t('storage.configureConnection'), schema: getProviderSchema(type || "s3", isEditMode, effectiveTarget) },
          // Only include preview and review steps for import storages
          ...(effectiveTarget === "import"
            ? [{ title: t('storage.importSettingsPreview') }, { title: t('storage.reviewConfirm') }]
    ...
    useEffect(() => {
      ...
      const newSteps = isEditMode
        ? [
            {
              title: t('storage.configureConnection'),
