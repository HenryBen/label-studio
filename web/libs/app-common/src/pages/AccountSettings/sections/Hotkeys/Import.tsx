...
export const ImportDialog = ({ open, onOpenChange, onImport }: ImportDialogProps) => {
  ...
  const validateHotkey = (hotkey: unknown): void => {
    if (!hotkey || typeof hotkey !== "object") {
      throw new Error(t("hotkeys.import.errors.invalidHotkeyObject"));
    }
    ...
    if (missingFields.length > 0) {
      throw new Error(t("hotkeys.import.errors.missingFields", { fields: missingFields.join(", ") }));
    }
  };
  ...
  const handleImport = (): void => {
    try {
      ...
      if (!importText.trim()) {
        throw new Error(t("hotkeys.import.errors.emptyData"));
      }
      ...
      if (Array.isArray(parsedData)) {
        ...
        if (!Array.isArray(dataObj.hotkeys)) {
          throw new Error(t("hotkeys.import.errors.invalidFormat"));
        }
      ...
      if (hotkeys.length === 0) {
        throw new Error(t("hotkeys.import.errors.noHotkeys"));
      }
      ...
      hotkeys.forEach((hotkey: unknown, index: number) => {
        try {
          ...
          const errorMessage = validationError instanceof Error ? validationError.message : t("common.unknownError");
          throw new Error(t("hotkeys.import.errors.hotkeyAtIndex", { index, message: errorMessage }));
        }
      });
        ...
        <div className="grid gap-4 py-4">
          ...
          {error && (
            <Alert variant="destructive" id="import-error">
              <AlertTitle>{t("hotkeys.import.errorTitle")}</AlertTitle>
