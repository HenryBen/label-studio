...
const createPanelView = (t: (key: string) => string): Record<PanelType, PanelView> => ({
  outliner: {
    title: t('sidePanels.outliner') || 'Outliner',
    component: OutlinerPanel as FC<PanelProps>,
    icon: IconHamburger,
  },
  details: {
    title: t('sidePanels.details') || 'Details',
...
const SidePanelsComponent: FC<SidePanelsProps> = ({ currentEntity, panelsHidden, children }) => {
  ...
  const panelView = useMemo(() => createPanelView(t), [t]);
