...
const TaskModal = observer(({ view, tasks, imageField, currentTaskId, setCurrentTaskId }: TaskModalProps) => {
  ...
  const tooltip = (
    <div className={styles.tooltip}>
      <p>{t('dataManager.gridPreview.description')}</p>
      <p>{t('dataManager.gridPreview.useArrowKeys')}</p>
      <p>{t('dataManager.gridPreview.escapeToClose')}</p>
      <p>{t('dataManager.gridPreview.spaceToSelect')}</p>
      <p>{t('dataManager.gridPreview.scrollAndDrag')}</p>
    </div>
  );
    ...
    <div className={styles.modal}>
      <div className={styles.header}>
        <Checkbox checked={view.selected.isSelected(task.id)} onChange={onSelect}>
          {t('dataManager.gridPreview.taskLabel', { id: task.id })}
        </Checkbox>
