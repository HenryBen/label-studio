...
const StorageProviderIcons = () => {
  ...
  return (
    <div className="flex items-center justify-center gap-base mb-wide" data-testid="dm-storage-provider-icons">
      <Tooltip title={t('dataImport.emptyState.storageProviders.amazonS3')}>
        <div className="flex items-center justify-center p-2" aria-label={t('dataImport.emptyState.storageProviders.amazonS3')}>
          <IconCloudProviderS3 width={32} height={32} className="text-neutral-content-subtler" />
        </div>
      </Tooltip>
      <Tooltip title={t('dataImport.emptyState.storageProviders.googleCloudStorage')}>
        <div className="flex items-center justify-center p-2" aria-label={t('dataImport.emptyState.storageProviders.googleCloudStorage')}>
          <IconCloudProviderGCS width={32} height={32} className="text-neutral-content-subtler" />
        </div>
      </Tooltip>
      <Tooltip title={t('dataImport.emptyState.storageProviders.azureBlobStorage')}>
        <div className="flex items-center justify-center p-2" aria-label={t('dataImport.emptyState.storageProviders.azureBlobStorage')}>
          <IconCloudProviderAzure width={32} height={32} className="text-neutral-content-subtler" />
        </div>
      </Tooltip>
      <Tooltip title={t('dataImport.emptyState.storageProviders.redisStorage')}>
        <div className="flex items-center justify-center p-2" aria-label={t('dataImport.emptyState.storageProviders.redisStorage')}>
...
const DocumentationLink = () => {
  ...
  return (
    <Typography variant="label" size="small" className="text-primary-link hover:underline">
      ...
      >
        {t('dataImport.emptyState.seeDocsOnImporting')}
        <span className="sr-only"> {t('dataImport.emptyState.opensInNewTab')}</span>
...
export const EmptyState: FC<EmptyStateProps> = ({
  ...
  if (hasFilters) {
    return renderEmptyStateLayout({
      ...
      title: t('dataManager.emptyState.noTasksFound'),
  ...
  if (userRole === "REVIEWER" || userRole === "ANNOTATOR") {
    ...
    if (userRole === "REVIEWER") {
      return renderEmptyStateLayout({
        icon: <IconCheck />,
        title: t('dataManager.emptyState.noTasksAvailableForReview'),
        description: t('dataManager.emptyState.tasksImportedWillAppear'),
      });
    }
    ...
    if (userRole === "ANNOTATOR") {
      ...
      if (isAutoDistribution) {
        return renderEmptyStateLayout({
          icon: <IconLsLabeling />,
          title: t('dataManager.emptyState.startLabelingTasks'),
          description: t('dataManager.emptyState.tasksLabeledWillAppear'),
          actions: (
            <Button
              variant="primary"
              look="filled"
            ...
            >
              {t('dataManager.emptyState.labelAllTasks')}
            </Button>
          ),
        });
      }
      ...
      if (isManualDistribution) {
        return renderEmptyStateLayout({
          icon: <IconInbox />,
          title: t('dataManager.emptyState.noTasksAvailable'),
          description: t('dataManager.emptyState.tasksAssignedWillAppear'),
        });
      }
      ...
      return renderEmptyStateLayout({
        ...
        title: t('dataManager.emptyState.noTasksAvailable'),
        description: t('dataManager.emptyState.tasksWillAppearWhenAvailable'),
      });
    }
  }
  ...
  return renderEmptyStateLayout({
    icon: <IconUpload />,
    title: t('dataImport.emptyState.title'),
    description: t('dataImport.emptyState.description'),
    ...
    actions: (
      <>
        ...
        >
          {t('dataImport.emptyState.connectCloudStorage')}
        </Button>
        ...
        {isImportEnabled && (
          ...
          >
            {t('dataImport.emptyState.import')}
          </Button>
        )}
      </>
    ),
