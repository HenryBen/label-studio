...
export const DataView = injector(
  ({
      ...
      (content) => {
          ...
          return (
            <Block name="syncInProgress">
              <Elem name="title" tag="h3">
                {t('dataManager.table.failedToSyncData')}
              </Elem>
              <Elem name="text">{t('dataManager.table.checkStorageSettings')}</Elem>
            </Block>
          );
        }
        if (
          ...
          return (
            <Block name="syncInProgress">
              <Elem name="title" tag="h3">
                {t('dataManager.table.nothingFound')}
              </Elem>
              <Elem name="text">{t('dataManager.table.tryAdjustingFilters')}</Elem>
            </Block>
          );
        }
        if (store.SDK.type === "DE" && (total === 0 || data.length === 0 || !hasData)) {
          return (
            <Block name="syncInProgress">
              <Elem name="title" tag="h3">
                {t('dataManager.table.recordsSyncing')}
              </Elem>
              <Elem name="text">{t('dataManager.table.pressButtonToSeeRecords')}</Elem>
              <Button
                size="small"
                look="outlined"
                onClick={async () => {
    ...
    useShortcut("hotkeys.dm:toggle-bulk-sidebar-minimization", () => {
      ...
      if (viewsStore && viewsStore.toggleSidebar) {
